    ...
    def generate_output_sentences(self, data_args: DataTrainingArguments, model, device, batch_size: int) \
        ...
        for i, inputs in tqdm(enumerate(test_data_loader), total=len(test_data_loader)):
            ...
            for j, (input_ids, label_ids, prediction) in enumerate(
                ...
                file = open("/content/drive/MyDrive/output_txt.txt", "a") #insert output file directory here
                info_dict = {"templates": [], "tokens": example.tokens}
                for i, trigger in enumerate(example.triggers):
                    template = {"incident_type": trigger.type.short, "PerpInd": [], "PerpOrg": [], "Target": [], "Victim": [], "Weapon": []}
                    for r in example.relations:
                        if r.tail.id == i:
                            relation_name = r.type.short
                            argument_type = relation_name[:relation_name.find(":")]
                            template[argument_type].append(example.tokens[r.head.start:r.head.end])
                    info_dict["templates"].append(template)
